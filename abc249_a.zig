...
fn calc (a: u64, b: u64, c: u64, x: u64) u64 {
  var i: u64 = 0;
  var result: u64 = 0;
...
pub fn main() !void {
  ...
  var i: u32 = 0;
  var input = [_]u64{0, 0, 0, 0, 0, 0, 0};
  
  while (itr.next()) |item| : (i += 1) {
    input[i] = try std.fmt.parseInt(u64, item, 10);
  }
  ...
  const f = input[5];
  const x = input[6];
  const t = calc(a, b, c, x);
  const ao = calc(d, e, f, x);
